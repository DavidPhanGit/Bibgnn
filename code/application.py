...
def a_t_collab_feature_setting():
    a_embed = np.around(np.random.normal(0, 0.01, [args.A_n, args.embed_d]), 4)
    t_embed = np.around(np.random.normal(0, 0.01, [args.T_n, args.embed_d]), 4)
    embed_f = open(args.data_path + "node_embedding.txt", "r")
    for line in islice(embed_f, 0, None):
        line = line.strip()
        node_id = re.split(' ', line)[0]
        if len(node_id) and (node_id[0] in ('a', 'p', 'v', 't')):
            type_label = node_id[0]
            index = int(node_id[1:])
            embed = np.asarray(re.split(' ', line)[1:], dtype='float32')
            if type_label == 'a':
    ...
    train_num = 0
    a_t_list_train_f = open(args.data_path + "a_t_list_train.txt", "r")
    a_t_list_train_feature_f = open(args.data_path + "train_feature.txt", "w")
    for line in a_t_list_train_f:
        line = line.strip()
        a = int(re.split(',', line)[0])
        t = int(re.split(',', line)[1])
        label = int(re.split(',', line)[2])
        if random.random() < 0.2:  # training data ratio
            train_num += 1
            a_t_list_train_feature_f.write("%d, %d, %d," % (a, t, label))
            for d in range(args.embed_d - 1):
                a_t_list_train_feature_f.write("%f," % (a_embed[a][d] * t_embed[t][d]))
            a_t_list_train_feature_f.write("%f" % (a_embed[a][args.embed_d - 1] * a_embed[t][args.embed_d - 1]))
            a_t_list_train_feature_f.write("\n")
    a_t_list_train_f.close()
    a_t_list_train_feature_f.close()
    # print train_num

    test_num = 0
    a_t_list_test_f = open(args.data_path + "a_t_list_test.txt", "r")
    a_t_list_test_feature_f = open(args.data_path + "test_feature.txt", "w")
    for line in a_t_list_test_f:
        line = line.strip()
        a = int(re.split(',', line)[0])
        t = int(re.split(',', line)[1])
        label = int(re.split(',', line)[2])
        test_num += 1
        a_t_list_test_feature_f.write("%d, %d, %d," % (a, t, label))
        for d in range(args.embed_d - 1):
            a_t_list_test_feature_f.write("%f," % (a_embed[a][d] * a_embed[t][d]))
        a_t_list_test_feature_f.write("%f" % (a_embed[a][args.embed_d - 1] * a_embed[t][args.embed_d - 1]))
        a_t_list_test_feature_f.write("\n")
    a_t_list_test_f.close()
    a_t_list_test_feature_f.close()
...
def t_t_recombination_feature_setting():
    # t_embed = np.around(np.random.normal(0, 0.01, [args.T_n, args.embed_d]), 4)
    t_embed = {}
    embed_f = open(args.data_path + "node_net_embedding.txt", "r")
    for line in islice(embed_f, 0, None):
        line = line.strip()
        node_id = re.split(' ', line)[0]
        if len(node_id) and (node_id[0] in ('a', 'p', 'v', 't')):
    ...
    train_num = 0
    t_t_list_train_f = open(args.data_path + "t_t_list_train.txt", "r")
    t_t_list_train_feature_f = open(args.data_path + "train_feature.txt", "w")
    for line in t_t_list_train_f:
        line = line.strip()
        a = int(re.split(',', line)[0])
        t = int(re.split(',', line)[1])
        label = int(re.split(',', line)[2])
        if random.random() < 0.2:  # training data ratio
            train_num += 1
            t_t_list_train_feature_f.write("%d, %d, %d," % (a, t, label))
            for d in range(args.embed_d - 1):
                t_t_list_train_feature_f.write("%f," % (t_embed[a][d] * t_embed[t][d]))
            t_t_list_train_feature_f.write("%f" % (t_embed[a][args.embed_d - 1] * t_embed[t][args.embed_d - 1]))
            t_t_list_train_feature_f.write("\n")
    t_t_list_train_f.close()
    t_t_list_train_feature_f.close()
    # print train_num

    test_num = 0
    t_t_list_test_f = open(args.data_path + "t_t_list_test.txt", "r")
    t_t_list_test_feature_f = open(args.data_path + "test_feature.txt", "w")
    for line in t_t_list_test_f:
        line = line.strip()
        a = int(re.split(',', line)[0])
        t = int(re.split(',', line)[1])
        label = int(re.split(',', line)[2])
        test_num += 1
        t_t_list_test_feature_f.write("%d, %d, %d," % (a, t, label))
        for d in range(args.embed_d - 1):
            t_t_list_test_feature_f.write("%f," % (t_embed[a][d] * t_embed[t][d]))
        t_t_list_test_feature_f.write("%f" % (t_embed[a][args.embed_d - 1] * t_embed[t][args.embed_d - 1]))
        t_t_list_test_feature_f.write("\n")
    t_t_list_test_f.close()
    t_t_list_test_feature_f.close()
...
#
# print("------author topic link prediction------")
# train_num, test_num = a_t_collab_feature_setting()  # setup of author-author collaboration prediction task
# LP.model(train_num, test_num)
# print("------author topic link prediction end------")

print("------topic topic link prediction------")
train_num, test_num = t_t_recombination_feature_setting()  # setup of author-author collaboration prediction task
LP.model(args.data_path, train_num, test_num)
print("------topic topic link prediction end------")

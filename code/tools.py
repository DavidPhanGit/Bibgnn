...
class HetAgg(nn.Module):
    ...
    def a_content_agg(self, id_batch):  # heterogeneous content aggregation
        ...
        concate_embed = torch.cat((
            a_net_embed_batch,
            # a_text_embed_batch_1,
            # a_text_embed_batch_2,
            # a_text_embed_batch_3
        ), 1).view(len(id_batch[0]), 1, embed_d)
    ...
    def p_content_agg(self, id_batch):
        ...
        p_t_net_embed_batch = self.feature_list[2][id_batch]
        p_net_embed_batch = self.feature_list[4][id_batch]

        concate_embed = torch.cat((
            p_v_net_embed_batch,
            p_a_net_embed_batch, p_t_net_embed_batch, p_net_embed_batch), 1).view(len(id_batch[0]), 4, embed_d)
    ...
    def v_content_agg(self, id_batch):
        embed_d = self.embed_d
        # print(id_batch)
        v_net_embed_batch = self.feature_list[6][id_batch]
        ...
        concate_embed = torch.cat((v_net_embed_batch,
                                   # v_text_embed_batch_1,
                                   # v_text_embed_batch_2,
                                   # v_text_embed_batch_3,
                                   # v_text_embed_batch_4,
                                   # v_text_embed_batch_5
                                   ),
                                  1).view(len(id_batch[0]), 1, embed_d)
    ...
    def t_content_agg(self, id_batch):
        embed_d = self.embed_d
        t_net_embed_batch = self.feature_list[7][id_batch]

        concate_embed = torch.cat((t_net_embed_batch,
                                   ),
                                  1).view(len(id_batch[0]), 1, embed_d)
    ...
    def node_het_agg(self, id_batch, node_type):  # heterogeneous neighbor aggregation
        ...
        for i in range(len(id_batch)):
            if node_type == 1:
                ...
                t_neigh_batch[i] = self.t_neigh_list_train[3][id_batch[i]]
            else:
        ...
        # print(c_agg_batch.shape, a_agg_batch.shape, p_agg_batch.shape, v_agg_batch.shape, t_agg_batch.shape)
        ...
        # compute weights
        concate_embed = torch.cat((c_agg_batch_2, a_agg_batch_2, p_agg_batch_2, v_agg_batch_2, t_agg_batch_2), 1).view(len(c_agg_batch), 5, self.embed_d * 2)
        if node_type == 1:
